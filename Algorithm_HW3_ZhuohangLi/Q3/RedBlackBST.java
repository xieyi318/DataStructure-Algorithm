...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    /***************************************************************************
     *  Methods to compute the percentage of red nodes.
     ***************************************************************************/
    public double redRatio() {
        if (n == 0) return 0;
        return countRed(root) * 1.0 / n;
    }

    private int countRed(Node x) {
        if (x == null) return 0;
        if (isRed(x)) return 1 + countRed(x.left) + countRed(x.right);
        return countRed(x.left) + countRed(x.right);
    }
    ...
    public static void main(String[] args) {
        ...
        String fileName = "redNodes.csv";
        String FILE_HEADER = "Data Size,Percentage of Red Nodes";
        ...
        try {
            ...
            for (int sz = 10000; sz <= 1000000; sz *= 10) {   //loop for data sizes from 10^4 to 10^6
                Integer[] keys = new Integer[sz];
                double redRatio=0;
                for (int trial = 1; trial <= 100; trial++) {    //loop for 100 trials for every size of data
                    System.out.println("Size:"+Integer.toString(sz)+"   Trial:"+Integer.toString(trial));
                    ...
                    redRatio += rb.redRatio();
                }
                redRatio = redRatio / 100.0;
                fileWriter.append(Integer.toString(sz));
                fileWriter.append(COMMA_DELIMITER);
                fileWriter.append(Double.toString(redRatio));
                fileWriter.append(NEW_LINE_SEPARATOR);
            }
